...
Задание: Построить график динамики суммарной массы поставок в сентябре-декабре (пропуски, заменённые на медианы, Netweight.kg.median) по годам. Цветом показать  три группы стран-поставщиков: страны Таможенного союза, остальные страны СНГ и  все остальные страны. 
...
```
Так как для задания нужны данные за сентябрь - декабрь, то выделяем нужные нам месяцы, т.е. больше 8 (= августа). Делим данные на три группы стран для дальнейшей работы: построения графиков.
```{r}
...
DT.import[, СтраныПоставщики := factor(group, levels = c(0, 1 , 2),
                                      labels = c('страны Таможенного союза',
                                                 ...
                                                 'остальные страны'))]
```
Построение графика динамики суммарной массы поставок в сентябре-декабре (пропуски, заменённые на медианы, Netweight.kg.median) по годам с помощью встроенной системы «base». На оси x мы показываем время, по оси y массы поставок.
```{r }
#Пакет "base"
# простой график разброса
plot(x = DT.import$Period, y = DT.import$Netweight.kg.median)
# берём палитру на 5 цветов
cls <- palette(rainbow(3))
# создаём график без осей
summary(DT.import$Period)
summary(DT.import$Netweight.kg.median)
countries <- as.factor(unique(DT.import$СтраныПоставщики))
plot(x = DT.import$Period , 
     y = DT.import$Netweight.kg.median, 
     pch = 21, 
     bg = cls[as.factor(DT.import$СтраныПоставщики)],
     axes = F, 
     ylim = c(0, 400000), 
     xlim = c(0, 15), 
...
axis(side = 2, 
     pos = 0, 
     at = seq(0, 350000, by = 50000),
     labels = c('', seq(50000, 350000, by = 50000)),
     las = 2)
# стрелки на концах осей
arrows(x0 = c(0, 12), y0 = c(350000, 0),
       x1 = c(0, 15), y1 = c(400000, 0))
...
mtext(text = '(в легенде указан номер страны)', 
